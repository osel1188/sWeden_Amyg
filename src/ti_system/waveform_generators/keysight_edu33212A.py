...
if __name__ == '__main__':
    ...
    # MODIFICATION: Changed the resource ID to use the pyvisa-py simulation backend.
    # The '@py' suffix tells PyVISA to use this backend, which will create a
    # simulated device if the hardware is not found.
    VISA_RESOURCE_ID = 'TCPIP0::localhost::inst0::INSTR@py'

    log.info("--- Starting Waveform Generator Example ---")

    try:
        # The driver is used as a context manager, ensuring connection and disconnection.
        with KeysightEDU33212A(resource_id=VISA_RESOURCE_ID) as generator:

            log.info("Generator connected. Applying waveform to Channel 1.")

            # Use the high-level convenience method from the abstract class
            generator.apply_waveform(
                channel=1,
                shape=WaveformShape.SINE,
                frequency=1000.0,  # 1 kHz
                amplitude=2.5,   # 2.5 Vpp
                offset=0.1       # 0.1 V DC offset
            )

            log.info("Waveform applied. Waiting for 5 seconds...")
            time.sleep(5)

            log.info("Getting current instrument status...")
            current_status = generator.get_status()

            # Pretty print the status dictionary
            import json
            print(json.dumps(current_status, indent=2))

            log.info("Turning off Channel 1 output.")
            generator.set_output_state(channel=1, state=OutputState.OFF)

            time.sleep(2)

    except ConnectionError as e:
        log.error(f"Could not run example. Hardware connection failed: {e}")
    except Exception as e:
        log.error(f"An unexpected error occurred during the example run: {e}")
